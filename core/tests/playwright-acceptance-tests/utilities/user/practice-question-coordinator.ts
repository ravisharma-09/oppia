...
export class QuestionCoordinator extends BaseUser {
  ...
  async addOrRemoveQuestionRightsInQuestionRoleEditorModal(
    ...
    if (!checkboxElement) {
      throw new Error(
        `Could not find checkbox for ${right} role before ${action}.`
      );
    }
   ...
   * @param number - The expected number of question reviewers.
   */
...
export const QuestionCoordinatorFactory = (page: Page): QuestionCoordinator => {
  return new QuestionCoordinator(page);
};
